         ...
         // label column 4: loot
            ...
            // find all inventory items first
               ...
               // label column 4: loot amount
                  ...
                  lootAmount.setHorizontalAlignment(SwingConstants.CENTER);    // centering label
                  lootAmount.setForeground(textColor);                         // changing color
   ...
   // Handles LootWindow buttons
      ...
      // Handles exiting loot selection screen
          public void actionPerformed(ActionEvent e) {
            ...
            lootWindow.dispose();
            Weapon bow = (Weapon) store.getLoot(new Weapon(GameMechanics.compoundBow));
            if (bow != null && bow.getAmount() != 0)
               if (Room.getArrowLevel() <= 0)
                  bow.setStrength(0);
               else
                  bow.setStrength(GameMechanics.BOWDAMAGE + GameMechanics.ARROWDAMAGE[Room.getArrowLevel() - 1]);
            store.updateArray();
